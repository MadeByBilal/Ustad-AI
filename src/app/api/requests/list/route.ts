...
import { authError, ok } from "@/lib/api";
...
export async function GET() {
  ...
  // Fetch all offers for these jobs
  const offers = await Offer.find({ job_id: { $in: jobIds } })
    .sort({ created_at: -1 })
    .lean();
  ...
  // Fetch worker names for jobs with selected workers
