...
import { validateCustomerOffer, validateWorkerCounter } from "@/server/lib/job/offers";
import { Job, JobEvent, Message, Offer, Worker, SYSTEM_SENDER_ID, type UrgencyLevel } from "@/server/models";
...
export async function createDirectRequest(
  ...
  if (!validation.valid) {
    throw new RequestError(
      validation.reason === "too_high" ? "offer_too_high" : "offer_too_low",
      validation.reason === "too_high"
        ? `Offer above the Rs ${validation.max_allowed} ceiling`
        : `Offer below the Rs ${validation.min_allowed} floor`,
      400
    );
  }
  ...
  const urgency = (input.understanding.urgency ?? "normal") as UrgencyLevel;
...
export async function respondToDirectRequest(
  workerUserId: string,
  offerId: string,
  input: WorkerRespondInput,
  now: Date = new Date()
): Promise<WorkerRespondResult> {
  await connectDB();
  ...
  if (job.matching?.acceptance_deadline && new Date(job.matching.acceptance_deadline as Date).getTime() < now.getTime()) {
    ...
    if (!check.valid) {
      throw new RequestError(
        ...
        check.reason === "too_low"
          ...
          : `Counter cannot exceed Rs ${check.max_allowed}`,
        400
      );
    }
  ...
  const jobUpdate = await Job.findOneAndUpdate(
    { _id: job._id, status: "BROADCASTING" },
    {
      $set: {
        status: "ACCEPTED",
  ...
  const lockResult = await Worker.updateOne(
    { _id: worker._id, active_job_id: null },
    { $set: { active_job_id: job._id, is_available: false } }
  );
  if (lockResult.matchedCount !== 1) {
    // Rollback
    await Job.findOneAndUpdate(
      { _id: job._id, status: "ACCEPTED" },
      { $set: { status: "BROADCASTING", "matching.selected_worker_id": null } }
    );
...
export async function respondToCounter(
  customerId: string,
  offerId: string,
  input: CounterResponseInput,
): Promise<CounterResponseResult> {
  await connectDB();
  ...
  const lockResult = await Worker.updateOne(
    { _id: offer.worker_id, active_job_id: null },
    { $set: { active_job_id: job._id, is_available: false } }
  );
  if (lockResult.matchedCount !== 1) {
    await Job.findOneAndUpdate(
