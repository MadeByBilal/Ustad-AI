...
import { getIO } from "@/server/lib/socket";
import type { UrgencyLevel, WorkerCategory } from "@/server/models";
...
export async function workerOffer(
  ...
  if (input.type === "decline") {
    ...
    return { job, offer: offer as unknown as { job_id: string; worker_id: string; type: WorkerOfferType; status: string; offered_price: number; message: string | null } };
  }
  ...
  return { job: claimed, offer: offer as unknown as WorkerOfferResult["offer"] };
}
