...
import { FlowError } from "../job/flow.js";
/**
...
export async function getWorkerDashboard(workerId) {
    ...
    const [workerLng, workerLat] = worker.location?.coordinates && worker.location.coordinates.length === 2
    ...
    const declinedJobIds = new Set(openOffers
    ...
    const incoming_jobs = incomingJobs
    ...
    const latestOfferByJob = new Map();
            ...
            ? {
                ...
                pricing: activeJob.pricing
                    ? {
                        ...
                        worker_counter_offer: activeJob.pricing.worker_counter_offer ?? undefined,
                ...
                route: activeJob.route
                    ? {
                        polyline: activeJob.route.polyline
                            ? Array.from(activeJob.route.polyline)
                            : null,
