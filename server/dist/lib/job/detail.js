 ...
 */
export async function getJobDetail(jobId, userId) {
    await connectDB();
    ...
    const [jobLng, jobLat] = job.location?.coordinates && job.location.coordinates.length === 2
        ...
        : [null, null];
    const responders = workers.map((w) => {
        ...
        if (jobLat != null &&
            ...
            distance_km = Number(haversineDistanceKm(jobLat, jobLng, w.location.coordinates[1], w.location.coordinates[0]).toFixed(2));
        }
        return {
            worker: {
                id: String(w._id),
