...
import { searchEligibleWorkers } from "../matching.js";
 ...
 */
async function recordSystemMessage(jobId, content) {
...
function formatPrice(rs) {
    return `Rs ${Math.round(rs).toLocaleString("en-PK")}`;
}
async function requireJob(filter) {
...
function guardJourney(job, from, to, actor) {
    if (job.status !== from || !canTransition(job.status, to, actor)) {
        throw new FlowError("invalid_status", `Job cannot move from ${job.status} to ${to}`, 409);
    }
}
function analysisTextFor(input) {
 ...
 */
export async function createAndAnalyzeJob(customerId, input) {
    ...
    const job = await Job.findOneAndUpdate({ _id: jobId, status: "DRAFT" }, { $set: { status: "WAITING_FOR_CUSTOMER" } }, { new: true });
    if (!job) {
 ...
 */
export async function reanalyzeJob(jobId, customerId, input) {
    ...
    const updated = await Job.findOneAndUpdate({ _id: jobId, customer_id: customerId, status: "WAITING_FOR_CUSTOMER" }, {
        $set: {
...
export async function confirmJobDetails(jobId, customerId) {
    const job = await requireJob({ _id: jobId, customer_id: customerId });
    guardJourney(job, "WAITING_FOR_CUSTOMER", "READY_TO_MATCH", "customer");
    const updated = await Job.findOneAndUpdate({ _id: jobId, customer_id: customerId, status: "WAITING_FOR_CUSTOMER" }, { $set: { status: "READY_TO_MATCH" } }, { new: true });
    if (!updated) {
 ...
 */
export async function submitOfferAndBroadcast(jobId, customerId, offerRs, now = new Date()) {
    const job = await requireJob({ _id: jobId, customer_id: customerId });
    guardJourney(job, "READY_TO_MATCH", "BROADCASTING", "customer");
    const urgency = job.understanding?.urgency ?? "normal";
    ...
    if (offerRs !== null && offerRs !== undefined) {
        const check = validateCustomerOffer(offerRs, job.pricing?.estimate_min ?? 0, job.pricing?.estimate_max ?? 0);
        if (!check.valid) {
            throw new FlowError(check.reason === "too_high" ? "offer_too_high" : "offer_too_low", check.reason === "too_high"
                ? `Offer above the Rs ${check.max_allowed} ceiling`
                : `Offer below the Rs ${check.min_allowed} floor`, 400);
        }
    }
    const [lng, lat] = job.location?.coordinates && job.location.coordinates.length === 2
        ? job.location.coordinates
        : [0, 0];
    const ranked = await searchEligibleWorkers({
        category: (job.understanding?.category ?? "plumber"),
    ...
    const updated = await Job.findOneAndUpdate({ _id: jobId, customer_id: customerId, status: "READY_TO_MATCH" }, {
        $set: {
            status: "BROADCASTING",
...
async function rollbackClaim(jobId, workerId, target, urgency) {
    const revert = urgency === "emergency"
        ? {
            status: "BROADCASTING",
    ...
    await Job.findOneAndUpdate({ _id: jobId, status: target, "matching.accepted_worker_ids": workerId }, { $set: revert, $pull: { "matching.accepted_worker_ids": workerId } }, { new: true });
}
/**
 ...
 */
export async function workerAcceptJob(jobId, workerId, now = new Date()) {
    ...
    const urgency = job.understanding?.urgency ?? "normal";
    const target = urgency === "emergency" ? "ACCEPTED" : "WORKER_RESPONSES";
    const claimFilter = {
        _id: jobId,
        status: "BROADCASTING",
        "matching.acceptance_deadline": { $gte: now },
    };
    const claimSet = urgency === "emergency"
        ? {
            status: "ACCEPTED",
    ...
    const claimed = await Job.findOneAndUpdate(claimFilter, { $set: claimSet, $push: { "matching.accepted_worker_ids": workerId } }, { new: true });
    if (!claimed) {
        throw new FlowError("job_already_claimed", "Job was claimed by another worker", 409);
    }
    const lock = await Worker.updateOne({ _id: workerId, active_job_id: null }, { $set: { active_job_id: jobId, is_available: false } });
    ...
    await recordSystemMessage(jobId, urgency === "emergency"
 ...
 */
export async function workerOffer(jobId, workerId, input, now = new Date()) {
    const job = await requireJob({ _id: jobId });
    if (job.status !== "BROADCASTING") {
        throw new FlowError("invalid_status", "Job is not broadcasting", 409);
    }
    ...
    if (input.type === "counter_offer") {
        ...
        if (!check.valid) {
            throw new FlowError(check.reason === "too_low" ? "counter_too_low" : "counter_too_high", check.reason === "too_low"
                ? `Counter offer must be at least ${check.min_allowed}`
                : `Counter offer cannot exceed ${check.max_allowed}`, 400);
        }
    }
    ...
    const claimed = await Job.findOneAndUpdate({
        ...
        status: "BROADCASTING",
        "matching.acceptance_deadline": { $gte: now },
    }, { $set: claimSet, $push: { "matching.accepted_worker_ids": workerId } }, { new: true });
    if (!claimed) {
        throw new FlowError("job_already_claimed", "Job was claimed by another worker", 409);
    }
    const lock = await Worker.updateOne({ _id: workerId, active_job_id: null }, { $set: { active_job_id: jobId, is_available: false } });
    ...
    await recordSystemMessage(jobId, input.type === "counter_offer"
    ...
    return { job: claimed, offer: offer };
}
/**
 ...
 */
const STATUS_SYSTEM_MESSAGES = {
    EN_ROUTE: "On the way",
    ARRIVED: "Worker has arrived",
    IN_PROGRESS: "Work has started",
    AWAITING_CUSTOMER_CONFIRMATION: "Work complete — please confirm",
};
export async function workerUpdateJobStatus(jobId, workerId, status, note) {
 ...
 */
export async function workerAttachPhoto(jobId, workerId, input) {
    const job = await requireJob({
        _id: jobId,
        "matching.selected_worker_id": workerId,
    });
    ...
    const field = input.type === "before"
        ? "completion.before_photo_id"
        : "completion.after_photo_id";
    const updated = await Job.findOneAndUpdate({ _id: jobId, status: job.status, "matching.selected_worker_id": workerId }, {
        $set: {
 ...
 */
export async function customerSelectWorker(jobId, customerId, workerId, now = new Date()) {
    const job = await requireJob({ _id: jobId, customer_id: customerId });
    ...
    const agreedPrice = pendingOffer?.type === "counter_offer"
    ...
    const selected = await Job.findOneAndUpdate({
        ...
        $set: {
            ...
            pricing: {
                ...
                worker_counter_offer: pendingOffer?.type === "counter_offer"
    ...
    const lock = await Worker.updateOne({ _id: workerId, active_job_id: jobId }, { $set: { is_available: false } });
    if (lock.matchedCount !== 1) {
    ...
    if (others.length > 0) {
        await Worker.updateMany({ _id: { $in: others }, active_job_id: jobId }, { $set: { active_job_id: null, is_available: true } });
    }
    if (pendingOffer) {
        await Offer.updateOne({ job_id: jobId, worker_id: workerId, status: "pending" }, { $set: { status: "selected", expires_at: null } });
    }
    if (others.length > 0) {
        await Offer.updateMany({ job_id: jobId, worker_id: { $in: others }, status: "pending" }, { $set: { status: "declined", expires_at: null } });
    }
    await recordEvent(jobId, "CUSTOMER_SELECTING", "ACCEPTED", customerId, "customer", {
        worker_id: workerId,
        final_price: agreedPrice,
    });
    await recordSystemMessage(jobId, `Worker selected — job confirmed at ${formatPrice(agreedPrice)}`);
    return selected;
}
/**
 ...
 */
export async function customerRejectWorker(jobId, customerId, workerId, action = "close", now = new Date()) {
    const job = await requireJob({ _id: jobId, customer_id: customerId });
    if (job.status !== "WORKER_RESPONSES" &&
        job.status !== "CUSTOMER_SELECTING") {
        throw new FlowError("invalid_status", "Job is not awaiting selection", 409);
    ...
    const target = action === "rebroadcast" ? "READY_TO_MATCH" : "CANCELLED";
    guardJourney(job, job.status, target, "customer");
    const update = action === "rebroadcast"
        ? {
        ...
        : { status: "CANCELLED" };
    const updated = await Job.findOneAndUpdate({ _id: jobId, customer_id: customerId, status: job.status }, { $set: update }, { new: true });
    if (!updated) {
        throw new FlowError("invalid_status", "Job changed concurrently", 409);
    }
    await Offer.updateOne({ job_id: jobId, worker_id: workerId, status: "pending" }, { $set: { status: "declined", expires_at: null } });
    await Worker.updateOne({ _id: workerId, active_job_id: jobId }, { $set: { active_job_id: null, is_available: true } });
    await recordEvent(jobId, job.status, target, customerId, "customer", {
        worker_id: workerId,
        reason: action === "rebroadcast"
            ? "offer-rejected-rebroadcast"
            : "offer-rejected-close",
    });
    await recordSystemMessage(jobId, action === "rebroadcast"
 ...
 */
export async function workerCancelJob(jobId, workerId, note) {
    const job = await requireJob({
        _id: jobId,
        "matching.selected_worker_id": workerId,
    });
    ...
    const updated = await Job.findOneAndUpdate({ _id: jobId, status: job.status, "matching.selected_worker_id": workerId }, { $set: { status: "CANCELLED" } }, { new: true });
    if (!updated) {
        throw new FlowError("invalid_status", "Job changed concurrently", 409);
    }
...
export async function customerCancelJob(jobId, customerId, note) {
    ...
    const updated = await Job.findOneAndUpdate({ _id: jobId, status: job.status, customer_id: customerId }, { $set: { status: "CANCELLED" } }, { new: true });
    if (!updated) {
        throw new FlowError("invalid_status", "Job changed concurrently", 409);
    }
 ...
 */
export async function markExpired(jobId, now = new Date()) {
    const job = await requireJob({ _id: jobId });
    const target = expireIfDeadlinePassed(job.status, job.matching, now);
    if (!target) {
        return null;
    }
    const updated = await Job.findOneAndUpdate({ _id: jobId, status: job.status }, {
        $set: {
            status: "EXPIRED",
            "matching.acceptance_deadline": null,
            "matching.selection_deadline": null,
    ...
    if (responders.length > 0) {
        await Worker.updateMany({ _id: { $in: responders }, active_job_id: jobId }, { $set: { active_job_id: null, is_available: true } });
    }
