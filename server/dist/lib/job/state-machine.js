...
export function assertAllowedTransition(from, to, actor) {
    if (!canTransition(from, to, actor)) {
        throw new Error(`Invalid transition ${from} -> ${to} for actor ${actor}`);
    }
}
export function resolveAcceptanceDeadline(urgency, now = new Date()) {
    const minutes = urgency === "emergency"
        ? EMERGENCY_ACCEPTANCE_MINUTES
        : NORMAL_ACCEPTANCE_MINUTES;
    return new Date(now.getTime() + minutes * 60_000);
}
export function resolveSelectionDeadline(now = new Date()) {
 ...
 */
export function expireIfDeadlinePassed(status, deadlines, now = new Date()) {
    const acceptanceDeadline = deadlines?.acceptance_deadline
        ? new Date(deadlines?.acceptance_deadline)
        : null;
    const selectionDeadline = deadlines?.selection_deadline
        ? new Date(deadlines?.selection_deadline)
        : null;
    if ((status === "BROADCASTING" || status === "WORKER_RESPONSES") &&
        acceptanceDeadline &&
        now.getTime() > acceptanceDeadline.getTime()) {
        return "EXPIRED";
    }
    if (status === "WORKER_RESPONSES" &&
        selectionDeadline &&
        now.getTime() > selectionDeadline.getTime()) {
        return "EXPIRED";
    }
    if (status === "CUSTOMER_SELECTING" &&
        selectionDeadline &&
        now.getTime() > selectionDeadline.getTime()) {
        return "EXPIRED";
    }
