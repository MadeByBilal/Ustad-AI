...
import { Job, Message, User, Worker, } from "../../models/index.js";
 ...
 */
export async function getAccessibleJob(jobId, userId, role) {
    await connectDB();
...
export async function listJobMessages(jobId, userId, role) {
    ...
    for (const w of workers)
        names.set(String(w._id), w.name || "Ustad");
    for (const c of customers)
        names.set(String(c._id), c.name || "Customer");
    return messages.map((m) => ({
        id: String(m._id),
        sender_type: m.sender_type,
        sender_name: m.sender_type === "system"
            ? "Ustad AI"
            : (names.get(String(m.sender_id)) ?? (m.sender_type === "worker" ? "Ustad" : "Customer")),
        content: m.content ?? "",
        media_ids: m.media_ids ?? [],
        location: m.location && typeof m.location.lat === "number" && typeof m.location.lng === "number"
            ? { lat: m.location.lat, lng: m.location.lng }
            : null,
        created_at: new Date(m.created_at).toISOString(),
    }));
}
export async function sendJobMessage(jobId, senderId, senderType, input) {
