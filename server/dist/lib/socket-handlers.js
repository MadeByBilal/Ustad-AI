    ...
    io.on("connection", (socket) => {
        ...
        socket.on("worker-location", async (data) => {
            try {
                ...
                if (workerId) {
                    await Worker.findOneAndUpdate({ _id: workerId }, {
                        $set: {
                            "location.type": "Point",
                ...
                if (distanceKm <= ARRIVAL_THRESHOLD_KM && job.status === "EN_ROUTE") {
                    ...
                    if (workerId) {
                        ...
                        await Job.findOneAndUpdate({ _id: jobId, status: "EN_ROUTE" }, { $set: { status: "ARRIVED" } });
                        // Record job event
                        ...
                        console.log(`[socket] auto-arrival triggered for job ${jobId} at ${distanceMeters}m`);
                    }
                }
            }
            catch (err) {
