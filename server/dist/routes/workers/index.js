 ...
 */
router.get("/by-user/:userId", requireRole(["worker"]), async (req, res) => {
...
router.get("/nearby", async (req, res) => {
    try {
        ...
        const workers = (await Worker.find(query).limit(limit).lean()).filter((w) => {
            const loc = w.location;
            return Boolean(loc?.coordinates) && loc.coordinates.length === 2;
        });
        const results = workers
            ...
            distance_km: Number(haversineDistanceKm(lat, lng, w.location.coordinates[1], w.location.coordinates[0]).toFixed(2)),
        }))
 ...
 */
router.get("/search", requireRole(["customer", "worker"]), async (req, res) => {
    try {
 ...
 */
router.get("/:id/dashboard", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
 ...
 */
router.post("/me/photos", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
 ...
 */
router.patch("/me/location", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    ...
    try {
        ...
        const coordinates = [parsed.data.lng, parsed.data.lat];
        const hasServiceArea = worker.service_area?.type === "Polygon" &&
        ...
        const locationFilter = hasServiceArea
            ...
            : { _id: worker._id };
        const updated = await Worker.findOneAndUpdate(locationFilter, {
            $set: {
            ...
            if (io) {
                ...
                if (job) {
                    ...
                    if (target) {
                        ...
                        const distanceKm = haversineDistanceKm(parsed.data.lat, parsed.data.lng, targetLat, targetLng);
 ...
 */
router.patch("/me/profile", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    ...
    try {
        await connectDB();
        const worker = await Worker.findOneAndUpdate({ user_id: sessionUser.user._id }, { $set: parsed.data }, { new: true }).lean();
        if (!worker) {
            return fail(res, "Worker profile not found for this account", 404);
        }
        return ok({
 ...
 */
router.patch("/me/availability", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
 ...
 */
router.get("/:id/completed-jobs", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const workerId = String(req.params.id).trim();
    if (!workerId) {
        return fail(res, "Worker id is required", 400);
 ...
 */
router.get("/:id/reviews", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const workerId = String(req.params.id).trim();
    if (!workerId) {
        return fail(res, "Worker id is required", 400);
