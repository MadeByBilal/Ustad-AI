...
import { FlowError, reanalyzeJob, workerAcceptJob, workerOffer, workerUpdateJobStatus, workerCancelJob, customerCancelJob, confirmJobDetails, submitOfferAndBroadcast, customerRejectWorker, customerSelectWorker, workerAttachPhoto, WORKER_SCORE_COMPLETION_REWARD, } from "../../lib/job/flow.js";
...
const postSchema = z
    ...
    .refine((v) => Boolean(v.content && v.content.length > 0) ||
    Boolean(v.photo_ids && v.photo_ids.length > 0) ||
    v.location != null, { message: "Send text, a photo or a location" });
/**
 * GET /:id — single-job view used by the customer results screen and the worker feed.
 */
router.get("/:id", requireRole(["customer", "worker"]), async (req, res) => {
 ...
 */
router.patch("/:id", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/accept", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
...
router.post("/:id/decline", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/offer", requireRole(["customer"]), async (req, res) => {
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    }
    ...
    try {
        ...
        const offer = await Offer.findOneAndUpdate({ job_id: jobId, worker_id: parsed.data.worker_id }, {
            job_id: jobId,
 ...
 */
router.post("/:id/confirm", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/cancel", requireRole(["worker", "customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/status", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        ...
        const job = await workerUpdateJobStatus(jobId, String(worker._id), parsed.data.status, parsed.data.note);
        return ok(job)(res);
    }
    catch (e) {
        if (e instanceof FlowError) {
 ...
 */
router.post("/:id/broadcast", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        const broadcast = await submitOfferAndBroadcast(jobId, String(sessionUser.user._id), parsed.data.offer_rs ?? null);
        return ok({
            job: broadcast.job,
 ...
 */
router.post("/:id/select-worker", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        const job = await customerSelectWorker(jobId, String(sessionUser.user._id), parsed.data.worker_id);
        return ok(job)(res);
    }
    catch (e) {
        if (e instanceof FlowError) {
 ...
 */
router.post("/:id/reject-offer", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        const job = await customerRejectWorker(jobId, String(sessionUser.user._id), parsed.data.worker_id, parsed.data.action);
        return ok(job)(res);
    }
    catch (e) {
        if (e instanceof FlowError) {
 ...
 */
router.post("/:id/approve", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        ...
        const updated = await Job.findOneAndUpdate({ _id: jobId, status: "AWAITING_CUSTOMER_CONFIRMATION" }, {
            $set: {
                status: targetStatus,
        ...
        const messageContent = parsed.data.action === "approve"
        ...
        if (targetStatus === "COMPLETED" && job.matching?.selected_worker_id) {
            ...
            const releasedWorker = await Worker.findOneAndUpdate({ _id: workerId, active_job_id: jobId }, {
            ...
            if (parsed.data.action === "approve") {
                await Worker.findOneAndUpdate({ _id: workerId }, { $inc: { confirmed_jobs: 1 } });
            }
        }
 ...
 */
router.post("/:id/review", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
    ...
    try {
        ...
        await Worker.findOneAndUpdate({ _id: workerId }, { $set: { average_rating: Math.round(avgRating * 10) / 10 } });
 ...
 */
router.post("/:id/media", requireRole(["worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.get("/:id/messages", requireRole(["customer", "worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/messages", requireRole(["customer", "worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.get("/:id/tracking", requireRole(["customer", "worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    try {
 ...
 */
router.patch("/:id/customer-location", requireRole(["customer"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
 ...
 */
router.get("/:id/stream", requireRole(["customer", "worker"]), async (req, res) => {
    const sessionUser = res.locals.sessionUser;
    const jobId = String(req.params.id).trim();
    if (!jobId) {
        return fail(res, "Job id is required", 400);
