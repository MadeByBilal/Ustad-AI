 ...
 */
router.post("/:id/inspection-offer", requireRole(["worker"]), async (req: Request, res: Response) => {
  const sessionUser = res.locals.sessionUser;
  const jobId = String(req.params.id).trim();
  if (!jobId) {
    return fail(res, "Job id is required", 400);
 ...
 */
router.post("/:id/inspection-offer/respond", requireRole(["customer"]), async (req: Request, res: Response) => {
  const sessionUser = res.locals.sessionUser;
  const jobId = String(req.params.id).trim();
  if (!jobId) {
    return fail(res, "Job id is required", 400);
